...
class TestTransactionWorkflow:
    ...
    def test_complete_transaction_flow(self, authenticated_client):
        ...
        assert transaction.risk_score == 0  # Initially 0, will be updated by event processor
    ...
    def test_transaction_status_update_creates_audit_log(self, authenticated_client, transaction):
        """Test that updating transaction status creates audit log"""
        initial_log_count = AuditLog.objects.count()

        url = reverse('transactions:transaction-update-status', kwargs={'pk': transaction.id})
    ...
    def test_blacklisted_customer_transaction(self, authenticated_client, blacklisted_customer):
