...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        for txn in transactions:
            customer_txn_count = Transaction.objects.filter(customer=txn.customer).count()
            customer_total_volume = Transaction.objects.filter(
        ...
        self.stdout.write(self.style.SUCCESS(f'Training model on {len(training_data)} samples...'))
        ...
        if result.get('success'):
            ...
            self.stdout.write(f"Train score: {result['train_score_mean']:.4f} ± {result['train_score_std']:.4f}")
            self.stdout.write(f"Validation score: {result['val_score_mean']:.4f} ± {result['val_score_std']:.4f}")
