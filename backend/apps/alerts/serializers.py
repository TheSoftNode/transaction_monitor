...
class AlertSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Alert
        fields = [
            'id', 'transaction', 'transaction_details', 'rule_name', 'severity',
            'message', 'status', 'resolved_by', 'resolved_by_username',
...
class AlertListSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Alert
        fields = [
            'id', 'transaction', 'transaction_reference', 'rule_name',
...
class AlertResolveSerializer(serializers.ModelSerializer):
    ...
    def validate_status(self, value):
        if value not in ['resolved', 'false_positive']:
            raise serializers.ValidationError("Status must be 'resolved' or 'false_positive'.")
        return value
...
class AuditLogSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = AuditLog
        fields = [
            'id', 'transaction', 'transaction_reference', 'event_type',
            'actor', 'actor_username', 'details', 'ip_address', 'user_agent', 'timestamp'
        ]
